...
public class GisUI extends UI implements ClickListener, CloseListener {
	 ...
	 * entity manager that should be used by this
	 * UI/user/"application instance".
	 * 
	 * @return
	 */
	...
	private void loadEvents() {
		...
		/* ... and map */
		Bounds boundsOfAllEvents = new Bounds();
		map.removeAllComponents();
		map.addBaseLayer(osmTiles, "OSM");
		for (final SpatialEvent spatialEvent : events) {
			AbstractLeafletLayer layer;
			/* Marker for point event, Polyline for events with route */
			if (spatialEvent instanceof EventWithPoint) {
				EventWithPoint p = (EventWithPoint) spatialEvent;
				Point point = JTSUtil.toLeafletPoint(p.getLocation());
				layer = new LMarker(point);
				boundsOfAllEvents.extend(point);
			} else {
				EventWithRoute r = (EventWithRoute) spatialEvent;
				Point[] pointArray = JTSUtil.toLeafletPointArray(r.getRoute());
				layer = new LPolyline(pointArray);
				boundsOfAllEvents.extend(pointArray);
			}
		...
		map.zoomToExtent(boundsOfAllEvents);
